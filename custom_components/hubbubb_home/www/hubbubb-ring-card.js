...
`).map(i=>v`<div class="${Ps(i)}">${i||" "}</div>`);if(t.role!=="assistant")return t.text;let e=[],s=0;for(let i of t.text.matchAll(zs))i.index>s&&e.push(t.text.slice(s,i.index)),e.push(i[1]?v`<code>${i[1]}</code>`:v`<b>${i[2]}</b>`),s=i.index+i[0].length;return e.push(t.text.slice(s)),e}_sameMsgs(t,e){if(!t||!e||t.length!==e.length)return!1;let s=t[t.length-1],i=e[e.length-1];return s?.role===i?.role&&s?.text===i?.text}_collapse(t){return t.split(/\s+/).join(" ")}_composerEl(){return this.renderRoot?.querySelector(".composer textarea")}_growCap(){let t=this._vvH||window.visualViewport?.height||window.innerHeight;return Math.max(56,Math.min(190,Math.round(t*.32)))}_autoGrow(t){if(!t)return;let e=t.style.height,s=t.scrollTop;t.style.height="auto";let i=Math.min(t.scrollHeight,this._growCap())+"px";return t.style.height=i,t.scrollTop!==s&&(t.scrollTop=s),i!==e}_saveDraft(t=this._sel){if(!t)return;let e=this._composerEl();e&&this._store(`draft:${t}`,e.value||null),this._store(`files:${t}`,this._files?.length?this._files:null)}_restoreDraft(){if(!this._sel)return;this._files=this._recall(`files:${this._sel}`,12*3600*1e3)||[];let t=this._composerEl();if(!t||t.value)return;let e=this._recall(`draft:${this._sel}`,12*3600*1e3);e&&(t.value=e,this._autoGrow(t))}_saveQueue(){let t=(this._queue||[]).filter(e=>e.state==="held");this._store("queue",t.length?t:null)}_restoreQueue(){let t=this._recall("queue",216e5);Array.isArray(t)&&t.length&&(this._queue=t.map(e=>({...e,state:"held"})))}async _send(t){t.preventDefault();let e=this._composerEl(),s=e?.value.trim(),i=this._files||[];if(!s&&!i.length||this._sel==null)return;let n=[...i.map(a=>a.path),s].filter(Boolean).join(" ");e.value="",e.style.height="",this._files=[],this._saveDraft();let o={id:this._sel,text:n,at:Date.now(),state:"held"};this._queue=[...this._queue||[],o],this._saveQueue(),this._stick=!0,(this._sessions||[]).find(a=>a.id===this._sel)?.busy||this._dispatch(o)}async _dispatch(t){if(!this._pending){this._pending=!0,t.state="sent",this._queue=[...this._queue],this._saveQueue();try{await this._api("agent_prompt_direct",{id:t.id,text:t.text}),this._err=""}catch(e){this._err=this._errText(e),t.state="held",this._queue=[...this._queue],this._saveQueue()}this._pending=!1,this._poll()}}_editQueued(t){let e=this._composerEl();e&&(e.value=e.value.trim()?`${e.value.trim()} ${t.text}`:t.text,this._dropQueued(t),e.focus(),this._autoGrow(e),this._saveDraft())}_dropQueued(t){this._queue=(this._queue||[]).filter(e=>e!==t),this._saveQueue()}async _attach(t){let e=[...t.target.files||[]];return t.target.value="",this._ingest(e)}async _ingest(t){if(!(!t.length||!this._sel)){this._uploading=!0;for(let e of t)try{let{name:s,data:i}=await this._encodeFile(e),n=await this._api("agent_upload",{name:s,data:i});if(!n?.path)throw new Error(n?.detail||"upload refused");this._files=[...this._files||[],{name:e.name,path:n.path}],this._err=""}catch(s){this._err=`${e.name}: ${this._errText(s)}`}this._uploading=!1,this._saveDraft()}}_encodeFile(t){let e=i=>new Promise((n,o)=>{let a=new FileReader;a.onerror=()=>o(new Error("could not read that file")),a.onload=()=>n(String(a.result).split(",")[1]||""),a.readAsDataURL(i)});if(!t.type.startsWith("image/"))return e(t).then(i=>({name:t.name,data:i}));let s=1568;return new Promise((i,n)=>{let o=new Image,a=URL.createObjectURL(t);o.onerror=()=>{URL.revokeObjectURL(a),e(t).then(l=>i({name:t.name,data:l}),n)},o.onload=()=>{URL.revokeObjectURL(a);let l=Math.min(1,s/Math.max(o.width,o.height));if(l===1&&t.size<9e5)return e(t).then(c=>i({name:t.name,data:c}),n);let h=document.createElement("canvas");h.width=Math.round(o.width*l),h.height=Math.round(o.height*l),h.getContext("2d").drawImage(o,0,0,h.width,h.height),h.toBlob(c=>{if(!c)return n(new Error("could not encode that image"));let d=t.name.replace(/\.[^.]+$/,"")+".jpg";e(c).then(p=>i({name:d,data:p}),n)},"image/jpeg",.85)},o.src=a})}_dropFile(t){this._files=(this._files||[]).filter(e=>e!==t),this._saveDraft()}async _sendKey(t){if(this._sel){this._askSent=t;try{await this._api("agent_key",{id:this._sel,key:t}),this._err=""}catch(e){this._err=this._errText(e),this._askSent=null;return}this._poll()}}async _killSession(t){if(t){if(this._confirmKill!==t){this._confirmKill=t,clearTimeout(this._confirmT),this._confirmT=setTimeout(()=>this._confirmKill=null,4e3);return}clearTimeout(this._confirmT),this._confirmKill=null;try{await this._api("agent_kill",{id:t}),this._swipe=null,this._store(`draft:${t}`,null),this._queue=(this._queue||[]).filter(e=>e.id!==t),this._saveQueue(),this._sel===t&&(this._sel=null,this._msgs=null,this._store("sel",null)),this._poll()}catch(e){this._err=this._errText(e)}}}_rowTap(t){if(this._swipe===t){this._swipe=null,this._confirmKill=null;return}if(this._swipe){this._swipe=null,this._confirmKill=null;return}this._swiped||this._select(t)}_swipeStart(t,e){let s=t.touches?.[0];s&&(this._sx=s.clientX,this._sy=s.clientY,this._swiped=!1)}_swipeMove(t,e){let s=t.touches?.[0];if(!s||this._sx==null)return;let i=s.clientX-this._sx,n=s.clientY-this._sy;Math.abs(n)>Math.abs(i)||(i<-12?(this._swiped=!0,this._swipe!==e&&(this._swipe=e)):i>12&&this._swipe===e&&(this._swiped=!0,this._swipe=null,this._confirmKill=null))}_swipeEnd(){this._sx=this._sy=null,setTimeout(()=>this._swiped=!1,50)}async _newSession(t){this._picking=!1,this._pending=!0;try{await this._api("agent_start_session",{project:t}),this._err="";for(let e=0;e<20;e++){await this._poll();let s=(this._sessions||[]).find(i=>i.target);if(s){this._select(s.id);break}await new Promise(i=>setTimeout(i,600))}}catch(e){this._err=this._errText(e)}this._pending=!1}_openAssist(){let t=this._hass?.auth?.external;if(t?.config?.hasAssist){t.fireMessage({type:"assist/show"});return}oe(this,"show-dialog",{dialogTag:"ha-voice-command-dialog",dialogImport:()=>customElements.whenDefined("ha-voice-command-dialog"),dialogParams:{pipeline_id:"last_used",start_listening:!0}})}_wakePuck(){let t;try{t=this._hass?.callService("assist_satellite","start_conversation",{entity_id:this._config.entity,start_message:this._config.tap_message,preannounce:!1},void 0,!1)}catch{return this._openAssist()}Promise.resolve(t).catch(()=>this._openAssist())}async _startLocalVoice(t="converse"){if(this._voice)return!0;let e;try{e=await navigator.mediaDevices.getUserMedia({audio:!0})}catch{return"microphone permission denied"}let s=window.AudioContext||window.webkitAudioContext,i;try{i=new s({sampleRate:16e3})}catch{i=new s}let n=null;t==="converse"&&(n=new Audio("data:audio/wav;base64,UklGRiQAAABXQVZFZm10IBAAAAABAAEAQB8AAIA+AAACABAAZGF0YQAAAAA="),n.play().catch(()=>{}));let o=this._voice={mode:t,stream:e,ctx:i,audio:n,proc:null,src:null,handlerId:null,unsub:null};t==="converse"?this._voiceState="listening":this._dictating=!0;let a=this._recall("conv",5*60*1e3);try{o.unsub=await this._hass.connection.subscribeMessage(c=>this._voiceEvent(c),{type:"assist_pipeline/run",start_stage:"stt",end_stage:t==="dictate"?"stt":"tts",input:{sample_rate:i.sampleRate},...a?{conversation_id:a}:{}})}catch{return this._stopLocalVoice(),"assist pipeline refused the run"}let l=o.src=i.createMediaStreamSource(e),h=o.proc=i.createScriptProcessor(2048,1,1);return h.onaudioprocess=c=>{if(o.handlerId==null)return;let d=c.inputBuffer.getChannelData(0),p=new Uint8Array(1+d.length*2);p[0]=o.handlerId;let f=new DataView(p.buffer,1);for(let _=0;_<d.length;_++){let g=Math.max(-1,Math.min(1,d[_]));f.setInt16(_*2,g<0?g*32768:g*32767,!0)}try{this._hass.connection.socket.send(p)}catch{}},l.connect(h),h.connect(i.destination),!0}_voiceEvent(t){let e=this._voice;if(e)switch(t.type){case"run-start":e.handlerId=t.data?.runner_data?.stt_binary_handler_id??null;break;case"intent-end":{let s=t.data?.intent_output?.conversation_id;s&&this._store("conv",s);break}case"stt-end":if(e.mode==="dictate"){let s=t.data?.stt_output?.text||"",i=this.renderRoot?.querySelector(".composer textarea");i&&s&&(i.value=(i.value?i.value.trim()+" ":"")+s,i.focus()),s||this._toast("Didn't catch that."),this._stopLocalVoice()}else this._closeMic(),this._voiceState="processing";break;case"tts-end":{let s=t.data?.tts_output?.url;if(!s)return this._stopLocalVoice();this._voiceState="responding",e.audio.onended=()=>this._stopLocalVoice(),e.audio.onerror=()=>this._stopLocalVoice(),this._envEl=e.audio,this._envUrl=s,this._env=null,this._decodeAudio(s),e.audio.src=s,e.audio.play().catch(()=>this._stopLocalVoice());break}case"error":e.mode==="dictate"&&this._toast("Didn't catch that."),this._stopLocalVoice();break;case"run-end":this._voiceState!=="responding"&&this._stopLocalVoice();break}}_closeMic(){let t=this._voice;t&&(t.proc?.disconnect(),t.src?.disconnect(),t.stream?.getTracks().forEach(e=>e.stop()),t.ctx?.close().catch(()=>{}),t.proc=t.src=null)}_stopLocalVoice(){let t=this._voice;if(t){this._closeMic(),this._dictating=!1;try{t.audio?.pause()}catch{}try{t.unsub?.()}catch{}this._voice=null,this._voiceState=null,this._envEl=null,this._env=null,this._envUrl=null}}_toast(t){oe(this,"hass-notification",{message:t})}get _canMic(){return!!(window.isSecureContext&&navigator.mediaDevices?.getUserMedia)}async _composerMic(){if(this._voice)return this._voice.mode==="dictate"?this._endSpeech():this._stopLocalVoice();if(!window.isSecureContext||!navigator.mediaDevices?.getUserMedia){this._toast("Voice input needs the https (Nabu Casa) URL.");return}let t=await this._startLocalVoice("dictate");t!==!0&&this._toast(`Couldn't start voice input (${t}).`)}_endSpeech(){let t=this._voice;if(!t||t.handlerId==null)return this._stopLocalVoice();t.proc?.disconnect(),t.src?.disconnect(),t.mode==="converse"&&(this._voiceState="processing");try{this._hass.connection.socket.send(new Uint8Array([t.handlerId]))}catch{this._stopLocalVoice()}}async _ringTap(){if(this._voice)return this._voiceState==="listening"?this._endSpeech():this._stopLocalVoice();if(this._effectiveState()==="responding"){let e=this._resolveMediaPlayer(this._hass);if(e){this._hass.callService("media_player","media_stop",{entity_id:e});return}}if(!window.isSecureContext){this._wakePuck();return}if(!navigator.mediaDevices?.getUserMedia){this._toast(`This browser exposes no microphone \u2014 waking ${this._name} instead.`),this._wakePuck();return}let t=await this._startLocalVoice();t!==!0&&(this._toast(`Couldn't use this device's mic (${t}) \u2014 waking ${this._name} instead.`),this._wakePuck())}firstUpdated(){this._setupCanvas()}updated(t){if(this._setupCanvas(),t.has("_sel")&&this._sel&&this._restoreDraft(),t.has("_msgs")&&this._stick){let e=this.renderRoot.querySelector(".log");e&&(e.scrollTop=e.scrollHeight)}}_setupCanvas(){let t=this.renderRoot?.querySelector("canvas");if(!t){this._stop(),this._canvas=null;return}if(t!==this._canvas){this._canvas=t;try{this._ctx=t.getContext("2d")}catch{this._ctx=null}if(!this._ctx)return;this._t=0,this._waveT=0,this._churnT=0,this._peaks=Is.map(e=>({...e})),this._spin=0,this._head=0,this._speech={t:0,next:0,env:0,count:0,syl:null,pulses:[],spikes:[]},this._cur={...Ye.idle},this._rgb=ts(this._config?.idle_color??"#3db4c8"),this._observe()}this._ctx&&(this._measure(),this._pump())}_observe(){typeof ResizeObserver<"u"&&(this._ro?.disconnect(),this._ro=new ResizeObserver(()=>this._measure()),this._ro.observe(this._canvas)),typeof IntersectionObserver<"u"?(this._io?.disconnect(),this._io=new IntersectionObserver(t=>{this._onScreen=t.some(e=>e.isIntersecting),this._pump()}),this._io.observe(this._canvas)):this._onScreen=!0}_measure(){let t=this._canvas;if(!t||!this._ctx)return;let e=t.clientWidth||Number(this._config?.size)||240,s=t.clientHeight||e,i=(this._perf?.q??1)<.75?1:2,n=X(window.devicePixelRatio||1,1,i);this._w===e&&this._h===s&&this._dpr===n&&this._particles||(this._w=e,this._h=s,this._dpr=n,t.width=Math.round(e*n),t.height=Math.round(s*n),this._ctx.setTransform(n,0,0,n,0,0),this._buildField())}_buildField(){let t=Number(this._config?.particles)||0,e=X(Math.round((this._w||240)*32),9e3,2e4),s=t>0?X(t,0,8e3):e,i=Math.max(400,Math.round(s*(this._perf?.q??1))),n=le(24301),o=new Array(i),a=0,l=Math.round(i*Tt[0].share);for(let h=0;h<i;h++){l--<=0&&a<Tt.length-1&&(a++,l=Math.round(i*Tt[a].share));let c=(n()+n()-1)*(n()<.04?4:1),d=(n()+n()-1)*1.05;o[h]={ri:a,lon:n()*Math.PI*2,sinLat:Math.sin(d),cosLat:Math.cos(d),off:c,drift:(n()-.5)*.13,tw:.5+n()*2,ph:n()*Math.PI*2,bobA:.1+n()*.25,bobW:.25+n()*.85,bobP:n()*Math.PI*2,wobA:.006+n()*.02,wobW:.4+n()*1.2,wobP:n()*Math.PI*2,lf:5+n()*7,lt:n()*12,sz:n()<.03?2.1+n()*1.1:.8+n()*.85,halo:n()<.05,br:n()<.04?1:.45+n()*.55}}this._particles=o}_audioSync(t){if(!t||t.state!=="playing"){this._env=null,this._envUrl=null;return}let e=t.attributes||{},s=String(e.media_content_id||""),i=/^https?:\/\//.test(s)||s.startsWith("/")?s:null,n=Date.parse(e.media_position_updated_at||t.last_changed||"");this._envAt=Number.isFinite(n)?n:Date.now(),this._envPos=Number(e.media_position)||0,!(!i||i===this._envUrl)&&(this._envUrl=i,this._env=null,i&&!(Number(e.media_duration)>60)&&this._decodeAudio(i))}async _pipelineId(t){if(this._pipeId!==void 0)return this._pipeId;this._pipeId=null;let e=await t.connection.sendMessagePromise({type:"assist_pipeline/pipeline/list"}),s=await t.connection.sendMessagePromise({type:"assist_pipeline/device/list"}),i=t.entities?.[this._config.entity]?.device_id,n=s?.find(h=>h.device_id===i)?.pipeline_entity,o=n?t.states?.[n]?.state:null,a=e?.pipelines||[],l=a.find(h=>h.name===o)||a.find(h=>h.id===e?.preferred_pipeline);return this._pipeId=l?.id||null,this._pipeId}async _grabTtsUrl(){let t=this._hass;if(!(!t?.connection||this._grabbing)){this._grabbing=!0;try{let e=await this._pipelineId(t);if(!e)return;for(let s=0;s<3;s++){let n=((await t.connection.sendMessagePromise({type:"assist_pipeline/pipeline_debug/list",pipeline_id:e}))?.pipeline_runs||[]).reduce((o,a)=>!o||a.timestamp>o.timestamp?a:o,null);if(n&&Date.now()-Date.parse(n.timestamp)<6e4){let a=(await t.connection.sendMessagePromise({type:"assist_pipeline/pipeline_debug/get",pipeline_id:e,pipeline_run_id:n.pipeline_run_id}))?.events||[];for(let l=a.length-1;l>=0;l--){let h=a[l]?.type==="tts-end"&&a[l]?.data?.tts_output?.url;if(h){this._envUrl=h,this._env=null,await this._decodeAudio(h);return}}}await new Promise(o=>setTimeout(o,250))}}catch{this._pipeId=void 0}finally{this._grabbing=!1}}}async _decodeAudio(t){try{let e=await(await fetch(t)).arrayBuffer(),s=window.AudioContext||window.webkitAudioContext;if(!s)return;let n=await(this._actx||(this._actx=new s)).decodeAudioData(e);if(t!==this._envUrl)return;this._env=Qe(n.getChannelData(0),n.sampleRate,es)}catch{this._env=null}}_audioLevel(){let t=this._env,e=this._envEl;if(e)return e.currentTime?t?Ge(t,e.currentTime):null:0;if(!t||this._mp!=="playing")return null;let s=this._envPos+(Date.now()-this._envAt)/1e3+(Number(this._config.audio_offset)||0);return Ge(t,s)}_sylShapes(t,e){let s=this._speech;s.pulses.push({born:s.t,amp:t}),s.pulses.length>6&&s.pulses.shift();let i=Math.random()*6.283185;s.spikes.push({a:i,sg:.1+Math.random()*.08,h:.06+t*.06,born:s.t,dur:.12+Math.random()*.08},{a:i+(Math.random()-.5)*.6,sg:.22+Math.random()*.18,h:.04+t*.05,born:s.t+.04,dur:e*(.9+Math.random()*.4)}),s.spikes.length>14&&s.spikes.splice(0,s.spikes.length-14)}_speechTick(t){let e=this._speech;e.t+=t;let s=this._audioLevel();if(s!=null){s>.2&&s>(e.last||0)*1.4&&(e.count++,e.syl={start:e.t,dur:.22,peak:s,f:5+s*4},this._sylShapes(s,.22)),e.last=s,e.live=!0;let a=s>e.env?.05:.14;e.env+=(s-e.env)*(1-Math.exp(-t/a)),e.env>1&&(e.env=1);return}if(e.live=!1,e.t>=e.next){let a=.22+Math.random()*.26,l=.5+Math.random()*.5;e.syl={start:e.t,dur:a,peak:l,f:5+Math.random()*4},e.count++;let h=.02+Math.random()*.05;e.count%(5+Math.floor(Math.random()*6))===0&&(h+=.14+Math.random()*.26),e.next=e.t+a+h,this._sylShapes(l,a)}let i=0,n=e.syl;if(n){let a=(e.t-n.start)/n.dur;a>=0&&a<=1&&(i=n.peak*1.2*(.5-.5*Math.cos(2*Math.PI*a)))}let o=i>e.env?.22:.5;e.env+=(i-e.env)*(1-Math.exp(-t/o)),e.env>1&&(e.env=1)}_pulseTable(t){let e=this._ptab||(this._ptab=new Float32Array(tt));if(e.fill(0),t<.01)return e;let s=this._speech;for(let i of s.pulses){let n=s.t-i.born;if(n>2.4)continue;let o=.58+n*.26,a=i.amp*Math.exp(-n/1)*t;for(let l=0;l<tt;l++){let h=Je+l/(tt-1)*Ze-o;e[l]+=a*Math.exp(-(h*h)/.0055)}}return e}_peakTable(t,e,s){let i=this._ktab||(this._ktab=new Float32Array(et));i.fill(0);let n=this._t,o=.05*e.amp+.08*s;for(let l of this._peaks){l.a+=t*l.w*(.6+e.swirl);let h=.35+.65*(.5+.5*Math.sin(n*l.f+l.ph)),c=l.h*h*o;if(c<.002)continue;let d=l.sg*(1-.4*s),p=2*d*d;for(let f=0;f<et;f++){let _=f/et*6.283185-l.a;_=(_%6.283185+6.283185)%6.283185,_>3.141593&&(_-=6.283185),i[f]+=c*Math.exp(-(_*_)/p)}}let a=this._speech;for(let l=a.spikes.length-1;l>=0;l--){let h=a.spikes[l],c=a.t-h.born;if(c>h.dur+.3){a.spikes.splice(l,1);continue}if(c<0)continue;let d=c<.12?c/.12:1,p=c>h.dur?Math.exp(-(c-h.dur)/.3):1,f=h.h*d*p*e.speech;if(f<.003)continue;let _=2*h.sg*h.sg;for(let g=0;g<et;g++){let x=g/et*6.283185-h.a;x=(x%6.283185+6.283185)%6.283185,x>3.141593&&(x-=6.283185),i[g]+=f*Math.exp(-(x*x)/_)}}return i}_sprite(t,e,s){let i=t<<16|e<<8|s;if(this._sprKey===i)return this._spr;let n=this._spr||(this._spr=document.createElement("canvas"));n.width=n.height=32;let o=n.getContext("2d");o.clearRect(0,0,32,32);let a=o.createRadialGradient(16,16,0,16,16,16);return a.addColorStop(0,`rgba(${t},${e},${s},0.75)`),a.addColorStop(.4,`rgba(${t},${e},${s},0.3)`),a.addColorStop(1,`rgba(${t},${e},${s},0)`),o.fillStyle=a,o.fillRect(0,0,32,32),this._sprKey=i,n}_segTick(t){if(this._segRot===void 0&&(this._segRot=-Math.PI/2),this._segRot+=t*.019,this._segWave=(this._segWave||0)+t*.5,this._sessions==null)return this._ambientTick(t);let e=this._segMap||(this._segMap=new Map),s=1-Math.exp(-t/.7),i=new Set;for(let n of this._sessions){i.add(n.id);let o=e.get(n.id);o||(o={size:.02,fill:Math.random(),pulse:Math.random()*6.28,rgb:[46,157,245],gone:!1},e.set(n.id,o)),o.weight=Math.max(.8,Math.log10((n.bytes||0)+1e4)-3.2),o.busy=!!n.busy,o.done=!!n.done,o.gone=!1}for(let[n,o]of e){if(i.has(n)||(o.gone=!0),o.size+=((o.gone?0:o.weight)-o.size)*s,o.gone&&o.size<.03){e.delete(n);continue}o.pulse+=t*(o.busy?2.4:.9),o.busy&&(o.fill=(o.fill+t*.3)%1);let a=o.done?[40,226,138]:[46,157,245];for(let l=0;l<3;l++)o.rgb[l]+=(a[l]-o.rgb[l])*s}}_segBand(t){let e=Math.abs(-Math.sin(t)-Math.sin(this._segWave||0));return e>1?0:.5+.5*Math.cos(e*Math.PI)}_wash(t,e,s,i,n,o,a,l){let h=Math.max(2,Math.min(12,Math.ceil((a-o)/.1))),c=(a-o)/h;for(let d=0;d<h;d++){let p=o+d*c,f=Math.min(a,p+c);t.globalAlpha=l*(.5+1.15*this._segBand(p+c/2)),t.beginPath(),t.arc(e,s,n,p,f),t.arc(e,s,i,f,p,!0),t.closePath(),t.fill()}}_drawSegs(t,e,s,i,n,o,a){if(this._sessions==null)return this._drawAmbientSegs(t,e,s,i,n,o,a);let l=6.283185,h=i*.475,c=i*.655,d=(g,x,C,y)=>{t.beginPath(),t.arc(e,s,y,g,x),t.arc(e,s,C,x,g,!0),t.closePath()},p=this._segMap,f=0;if(p)for(let g of p.values())f+=g.size;if(!p||!p.size||f<=0){t.fillStyle=`rgb(${n},${o},${a})`,t.globalAlpha=.05,d(0,l,h,c),t.fill(),t.globalAlpha=1;return}let _=this._segRot;for(let g of p.values()){let x=g.size/f*l,C=Math.min(.035,x),y=_+C/2,w=_+x-C/2;_+=x;let E=w-y;if(!(E<.004)&&(t.fillStyle=`rgb(${Math.round(g.rgb[0])},${Math.round(g.rgb[1])},${Math.round(g.rgb[2])})`,this._wash(t,e,s,h,c,y,w,(g.busy?.22:.1)+(g.done?.07:0)),g.busy)){let k=Math.max(.1,E*.35),P=y+g.fill*E,z=14;for(let b=0;b<z;b++){let S=P-b*k/z,R=Math.max(y,S-k/z);if(S<=y)break;t.globalAlpha=.3*Math.pow(1-b/z,1.6),d(R,S,h,c),t.fill()}}}t.globalAlpha=1}_ambientTick(t){let e=this._segs;if(!e){e=this._segs=[],this._segRot=-Math.PI/2;for(let n=0;n<22;n++){let o=Math.random()<.75;e.push({size:.5+Math.random()*1.4,fill:o?1:Math.random()*.7,rate:.12+Math.random()*.3,state:o?"done":"load",doneT:Math.random()*9})}}let s=0,i=0;for(let n of e)n.state==="grow"?s++:n.state==="die"&&i++;if(!s&&e.length<26&&Math.random()<t*.5&&e.splice(Math.floor(Math.random()*(e.length+1)),0,{size:.02,target:.5+Math.random()*1.4,fill:0,rate:.12+Math.random()*.3,state:"grow",doneT:0}),i<2&&e.length>16){let n=null;for(let o of e)o.state==="done"&&o.doneT>8&&(!n||o.doneT>n.doneT)&&(n=o);n&&Math.random()<t*.25&&(n.state="die")}for(let n=e.length-1;n>=0;n--){let o=e[n];o.state==="grow"?(o.size+=t*.5,o.size>=o.target&&(o.size=o.target,o.state="load")):o.state==="load"?(o.fill+=t*o.rate,o.fill>=1&&(o.fill=1,o.state="done",o.doneT=0)):o.state==="done"?o.doneT+=t:(o.size-=t*.12,o.fill=Math.max(0,o.fill-t*.25),o.size<=.02&&e.splice(n,1))}}_drawAmbientSegs(t,e,s,i,n,o,a){let l=this._segs;if(!l||!l.length)return;let h=0;for(let y of l)h+=y.size;if(h<=0)return;let c=i*.475,d=i*.655,p=n*.5+74|0,f=o*.72+48|0,_=a*.8+82|0;t.fillStyle=`rgb(${p},${f},${_})`;let g=(y,w,E,k)=>{t.beginPath(),t.arc(e,s,k,y,w),t.arc(e,s,E,w,y,!0),t.closePath()},x=6.283185,C=this._segRot;for(let y=0;y<l.length;y++){let w=l[y],E=w.size/h*x,k=Math.min(.028,E),P=C+k/2,z=C+E-k/2;C+=E;let b=z-P;if(!(b<.004)&&(this._wash(t,e,s,c,d,P,z,.13),w.fill>.01)){let S=w.state==="done"?.14+.2*Math.exp(-w.doneT/2.2):.34;this._wash(t,e,s,c,d,P,P+b*w.fill,S*.8)}}t.globalAlpha=1}_pump(){this._ctx&&this.isConnected&&this._onScreen!==!1&&!document.hidden?this._start():this._stop()}_start(){if(this._raf)return;this._last=performance.now();let t=e=>{this._raf=requestAnimationFrame(t);let s=e-this._last,i=Math.min(.05,s/1e3);this._last=e,this._perfTick(s),this._draw(i)};this._raf=requestAnimationFrame(t)}_perfTick(t){if(t<=0||t>250)return;let e=this._perf||(this._perf={q:1,acc:0,n:0,hold:40});if(e.hold>0){e.hold--;return}if(e.acc+=t,++e.n<45)return;let s=e.acc/e.n;e.acc=0,e.n=0,s>26&&e.q>.2?this._setQuality(Math.max(.2,e.q*.65)):s<14&&e.q<1&&this._setQuality(Math.min(1,e.q*1.2))}_setQuality(t){let e=this._perf;t!==e.q&&(e.q=t,e.hold=40,this._particles=null,this._measure())}_stop(){this._raf&&cancelAnimationFrame(this._raf),this._raf=0}_targets(){let t=this._effectiveState(),e=t==="offline"?this._config.offline_color:this._config[Xe[t]];return{energy:Ye[t],rgb:ts(e)}}_seedBubbles(t){let e=[];for(let s=0;s<t;s++)e.push({a:Math.random()*Math.PI*2,rad:.05+Math.pow(Math.random(),1.7)*.16,off:(Math.random()-.5)*.16,spd:.5+Math.random()*.9,wob:Math.random()*Math.PI*2,wobF:.4+Math.random()*.7,drift:0});return e.sort((s,i)=>i.rad-s.rad)}_drawBubbles(t){let e=this._ctx,s=this._w,i=this._h;if(!s||!i)return;let n=s/2,o=i/2,a=Math.min(s,i)/2,{energy:l,rgb:h}=this._targets(),c=1-Math.exp(-t/.55),d=1-Math.exp(-t/.16),p=this._cur;for(let b of Object.keys(l)){let S=b==="speech"&&l[b]<p[b]?d:c;p[b]+=(l[b]-p[b])*S}for(let b=0;b<3;b++)this._rgb[b]+=(h[b]-this._rgb[b])*c;let f=Math.round(this._rgb[0]),_=Math.round(this._rgb[1]),g=Math.round(this._rgb[2]);l.speech>.5&&this._speech.env<.05&&(this._speech.next=this._speech.t,this._speech.pulses.length=0),p.speech>.005?this._speechTick(t):this._speech.env+=(0-this._speech.env)*c;let x=this._speech,C=.5+.5*Math.sin(x.t*.5+.8*Math.sin(x.t*.21)),y=x.live?.88:.62,w=(y*x.env+(1-y)*C)*p.speech;this._t+=t;let E=p.swirl/1.8,k=Math.max(10,Math.min(34,Math.round(a/9*(Number(this._config.particle_size)||1))));(!this._bubbles||this._bubbles.length!==k)&&(this._bubbles=this._seedBubbles(k)),e.clearRect(0,0,s,i),e.globalCompositeOperation="lighter";let P=a*.6,z=Math.max(1.2,a*.022);e.strokeStyle=`rgba(${f},${_},${g},${.1+w*.18})`,e.lineWidth=z*4,e.beginPath(),e.arc(n,o,P,0,Math.PI*2),e.stroke(),e.strokeStyle=`rgba(${f},${_},${g},${.5+w*.4})`,e.lineWidth=z,e.beginPath(),e.arc(n,o,P,0,Math.PI*2),e.stroke();for(let b of this._bubbles){b.a+=t*b.spd*(.16+E*.5+w*.55),b.wob+=t*b.wobF,b.drift+=(w*.09-b.drift)*c;let S=P*(1+b.off+b.drift)+Math.sin(b.wob)*a*.035,R=n+Math.cos(b.a)*S,N=o+Math.sin(b.a)*S,T=a*b.rad*(1+w*.22+Math.sin(b.wob*1.7)*.06);if(T<.6)continue;let Y=e.createRadialGradient(R-T*.3,N-T*.35,T*.1,R,N,T),bt=.5+w*.3;Y.addColorStop(0,`rgba(${Math.min(255,f+90)},${Math.min(255,_+90)},${Math.min(255,g+70)},${bt})`),Y.addColorStop(.55,`rgba(${f},${_},${g},${bt*.75})`),Y.addColorStop(1,`rgba(${f},${_},${g},0)`),e.fillStyle=Y,e.beginPath(),e.arc(R,N,T,0,Math.PI*2),e.fill(),e.strokeStyle=`rgba(${f},${_},${g},${.45+w*.3})`,e.lineWidth=Math.max(.6,T*.09),e.beginPath(),e.arc(R,N,T*.94,0,Math.PI*2),e.stroke(),T>3&&(e.fillStyle=`rgba(255,255,255,${.28+w*.22})`,e.beginPath(),e.arc(R-T*.34,N-T*.38,T*.17,0,Math.PI*2),e.fill())}e.globalCompositeOperation="source-over"}_draw(t){let e=this._ctx;if(!e)return;if(this._config.animation!=="jarvis-v1")return this._drawBubbles(t);if(!this._particles)return;let s=this._w,i=this._h,n=s/2,o=i/2,a=Math.min(s,i)/2,{energy:l,rgb:h}=this._targets(),c=1-Math.exp(-t/.55),d=1-Math.exp(-t/.16),p=this._cur;for(let u of Object.keys(l)){let $=u==="speech"&&l[u]<p[u]?d:c;p[u]+=(l[u]-p[u])*$}for(let u=0;u<3;u++)this._rgb[u]+=(h[u]-this._rgb[u])*c;let f=Math.round(this._rgb[0]),_=Math.round(this._rgb[1]),g=Math.round(this._rgb[2]);this._t+=t,this._waveT+=t*(.5+p.swirl*.9),this._spin+=t*(.09+p.swirl*.05),this._head+=t*(1.1+p.swirl*1.4),l.speech>.5&&this._cur.speech<.05&&(this._speech.next=this._speech.t,this._speech.pulses.length=0),p.speech>.005?this._speechTick(t):this._speech.env+=(0-this._speech.env)*c;let x=this._speech,C=.5+.5*Math.sin(x.t*.5+.8*Math.sin(x.t*.21)),y=x.live?.88:.62,w=(y*x.env+(1-y)*C)*p.speech,E=this._speech.syl?this._speech.syl.f:11;this._churnT+=t*(.55+p.swirl*.85+w*.7),this._waveT+=t*w*.8;let k=this._churnT,P=this._pulseTable(p.speech),z=this._peakTable(t,p,w);e.clearRect(0,0,s,i);let b=this._t,S=this._waveT,R=this._spin*.6,N=.955,T=.296,Y=.72+.28*Math.sin(b*.21+1.3)*Math.sin(b*.093+4.1),bt=.012*Math.sin(b*.16+.7),Pt=a*.38*1.16,he=a*.38*.7,is=Pt*Pt,ns=Pt-he,zt=.92,ce=.075;e.fillStyle=`rgb(${f},${_},${g})`;let os=this._sprite(f,_,g),Ot=this._bins||(this._bins=[]);for(let u=0;u<_t;u++)Ot[u]=new Path2D;let Rt=this._perf?.q??1,rs=1+(1-Rt)*.6,as=Rt>=.5,ls=(Number(this._config.particle_size)||1)*X(a/220,.75,1.8)*(1+(1-Rt)*.35);for(let u of this._particles){let $=Tt[u.ri],M=u.lon+R*$.rev+u.drift*k+u.wobA*(1+w*1)*(Math.sin(k*u.wobW+u.wobP)+.6*Math.sin(k*u.wobW*2.3+u.wobP*2.1)),j=Math.cos(M),W=Math.sin(M),Nt=u.cosLat*j,L=u.sinLat,mt=u.cosLat*W,hs=.45*Math.sin(5*M-S*1.4+L*2.3)+.3*Math.sin(8*M+S*1.05-L*3.1)+.3*Math.sin((Nt*.9+L*.32+mt*.28)*3.1+S*.9)+.2*Math.sin(13*M-S*2.1+L*1.8),cs=.45*Math.cos(5*M-S*1.4+L*2.3)+.3*Math.cos(8*M+S*1.05-L*3.1),ge=.5+.5*Math.sin(M-k*(.5+u.ri*.13)+u.ri*2.1),q=Y*(.45+1.3*ge*ge)*(p.turb+w*.6);q>1.1&&(q=1.1);let ds=Math.sin((Nt*.7-L*.6+mt*.4)*7.3+S*1.8),vt=hs*(.4+q)+ds*.38*q,be=u.off+u.bobA*(.7+.6*q)*(Math.sin(k*u.bobW+u.bobP)+.55*Math.sin(k*u.bobW*2.7+u.bobP*1.9)),qt=1-Math.min(1,Math.abs(be)),me=p.amp*(1+w*.4),I=$.r+p.radius+bt+w*.015+$.amp*me*vt+be*$.thick;vt>.75&&(I+=(vt-.75)*.04*q);let it=Nt,nt=L*N-mt*T,ps=L*T+mt*N,wt=Math.sqrt(it*it+nt*nt)||1e-4,ve=Math.atan2(nt,it),Dt=ve*.15915494%1;Dt<0&&(Dt+=1);let Bt=z[Dt*et|0];I+=Bt*(.35+.65*qt);let xt=(b+u.lt)%u.lf;xt/=u.lf,I+=(xt-.5)*.008;let Vt=6*xt*(1-xt);Vt>1&&(Vt=1);let us=.24+.76*qt*qt,D=u.br*$.glow*us*Vt*(.55+.45*Math.abs(Math.sin(b*u.tw*.6+u.ph)))*p.bright;D*=.68+.32*Math.sin(M*2.7+k*.45+u.ph*3);let Wt=wt*wt;if(D*=.09+1.6*Wt*Wt*Wt,D*=.62+.5*Math.max(0,vt),D*=.85+.3*q,D*=1+Bt*(4+w*6),p.sweep>.01){let A=(ve-this._head)%6.283185;A<0&&(A+=6.283185),A>3.141593&&(A-=6.283185),D*=1+Math.exp(-(A*A)/.25)*p.sweep*2.4}let yt=0;if(p.speech>.01){let A=(I-Je)/Ze*(tt-1);A=A<0?0:A>tt-1?tt-1:A|0,yt=P[A],I+=yt*.07,D*=1+yt*1.1+w*.7}I>zt&&(I=zt+ce*Math.tanh((I-zt)/ce));let we=I*a,xe=ps*.5+.5,ye=u.sz*ls*(.78+.32*xe)*(1+w*.25+yt*.5+Bt*(.8+w*2)),$e=a*$.amp*me*cs*(.5+q*.45),$t=we*it-nt/wt*$e,kt=we*nt+it/wt*$e,ot=1,ke=$t*$t+kt*kt;if(ke<is){if(ot=(Math.sqrt(ke)-he)/ns,ot<.02)continue;ot*=ot}let Me=X(D*rs*(.45+.65*xe)*ot,0,1);if(u.halo&&as){let A=ye*8;e.globalAlpha=Me*.22,e.drawImage(os,n+$t-A/2,o+kt-A/2,A,A)}let Se=ye*.56,Ae=n+$t,Ee=o+kt,Te=Ot[Math.min(_t-1,Me*_t|0)];Te.moveTo(Ae+Se,Ee),Te.arc(Ae,Ee,Se,0,6.283185)}for(let u=0;u<_t;u++)e.globalAlpha=(u+.5)/_t,e.fill(Ot[u]);e.globalAlpha=1,this._segTick(t),this._drawSegs(e,n,o,a,f,_,g);let Lt=a*.38,de=Math.cos(this._spin),pe=Math.sin(this._spin),ue=Math.cos(.35),fe=Math.sin(.35),_e=ae.verts,It=_e.length;(!this._proj||this._proj.length!==It*3)&&(this._proj=new Float64Array(It*3));let O=this._proj;for(let u=0;u<It;u++){let $=_e[u],M=$[0]*de+$[2]*pe,j=-$[0]*pe+$[2]*de,W=$[1]*ue-j*fe;O[u*3]=n+M*Lt,O[u*3+1]=o-W*Lt,O[u*3+2]=$[1]*fe+j*ue}e.save(),e.beginPath(),e.arc(n,o,Lt,0,Math.PI*2),e.clip();let Ut=ae.edges,Ht=X(p.bright*(1+w*.45),.3,1.9);for(let u=0;u<2;u++){let $=u===1;e.beginPath();for(let M=0;M<Ut.length;M+=2){let j=Ut[M]*3,W=Ut[M+1]*3;O[j+2]+O[W+2]>0===$&&(e.moveTo(O[j],O[j+1]),e.lineTo(O[W],O[W+1]))}e.strokeStyle=$?`rgba(220,246,255,${.4*Ht})`:`rgba(${f},${_},${g},${.13*Ht})`,e.lineWidth=Math.max(.35,a*($?.0032:.0024)),e.stroke()}e.fillStyle="rgba(235,252,255,1)";for(let u of Ls){let $=u.v*3;if(O[$+2]<=.15)continue;let M=Math.pow(Math.abs(Math.sin(b*u.tw+u.ph)),6)*Ht;if(M<.04)continue;e.globalAlpha=X(M,0,1);let j=Math.max(1,a*.008);e.fillRect(O[$]-j/2,O[$+1]-j/2,j,j)}e.globalAlpha=1,e.restore()}_fmtElapsed(t){if(t==null)return"";let e=Math.floor(t/60);return e<1?`${Math.floor(t)}s`:e<60?`${e}m`:`${Math.floor(e/60)}h ${e%60}m`}_chipProjects(){let t=n=>n.replace(/\b\w/g,o=>o.toUpperCase()),e=n=>typeof n=="string"?{project:n,label:t(n)}:n,s=this._config.build_projects,i=this._projects||[];return Array.isArray(s)&&s.length?s.map(e).filter(n=>!i.length||i.includes(n.project)):i.map(e)}_renderList(){let t=this._sessions;return v`
    ...
    `}render(){if(!this._config)return m;if(!this._hass?.states?.[this._config.entity])return v`
      ...
      `;let e=this._effectiveState(),s=e==="offline"?this._config.offline_color:this._config[Xe[e]],i=!!this._build,n=this._announce??null,o=this._messages??null;return v`
      <ha-card
  ...
  `);var Hs=[{name:"entity",required:!0,selector:{entity:{domain:Ct}}},{name:"size",selector:{number:{min:80,max:600,step:10,mode:"slider"}}},{name:"background",selector:{select:{mode:"dropdown",options:[{value:"dark",label:"Deep space (dark)"},{value:"card",label:"Normal card background"},{value:"transparent",label:"Transparent"}]}}},{name:"particles",selector:{number:{min:0,max:4e3,step:20,mode:"box"}}},{name:"particle_size",selector:{number:{min:.5,max:3,step:.1,mode:"slider"}}},{name:"tap_message",selector:{text:{}}},{name:"follow_media_player",selector:{boolean:{}}},{name:"audio_offset",selector:{number:{min:-2,max:2,step:.05,mode:"box"}}},{name:"media_player",selector:{entity:{domain:"media_player"}}},{name:"",type:"expandable",title:"Build mode",icon:"mdi:hexagon-multiple-outline",schema:[{name:"build_entity",selector:{entity:{domain:"input_boolean"}}}]},{name:"announce_entity",selector:{entity:{domain:"input_boolean"}}},{name:"messages_entity",selector:{entity:{domain:"input_boolean"}}},{name:"",type:"expandable",title:"Colors",icon:"mdi:palette",schema:[{name:"idle_color",selector:{text:{}}},{name:"listening_color",selector:{text:{}}},{name:"processing_color",selector:{text:{}}},{name:"responding_color",selector:{text:{}}},{name:"offline_color",selector:{text:{}}}]}],Ns={entity:"Assist satellite",size:"Ring size (px)",background:"Card background",particles:"Particle count (0 = auto)",particle_size:"Particle size",follow_media_player:"Animate while the device is playing audio",audio_offset:"Audio sync offset (seconds)",media_player:"Speaker entity (blank = same device)",tap_message:"Spoken reply when the ring is tapped",build_entity:"Build mode toggle helper (optional)",announce_entity:"Agent announcement toggle",messages_entity:"Hubbubb message toggle",idle_color:"Idle",listening_color:"Listening",processing_color:"Processing",responding_color:"Responding",offline_color:"Unavailable"},jt=class extends H{setConfig(t){this._config={...ne,...t}}render(){return this._config?v`
      <ha-form
        .hass=${this.hass}
        .data=${this._config}
        .schema=${Hs}
